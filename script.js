...
/**
 * Очищує та закриває всі модальні вікна.
 */
...
/**
 * Обробник натискання кнопки, що викликає модальне вікно.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Слухачі для кнопок відкриття модальних вікон
    ...
    // 2. Слухачі для кнопок закриття (x) та оверлею
    ...
    // 4. Додавання слухача для форми
    document.getElementById('registrationForm')?.addEventListener('submit', handleRegistration);
});


// --- ФУНКЦІЯ РЕЄСТРАЦІЇ (ІМІТАЦІЯ) ---
async function handleRegistration(e) {
    ...
    // Імітація запиту до API (затримка 1.5 секунди)
    await new Promise(resolve => setTimeout(resolve, 1500));

    // Імітація успішної відповіді
    if (name.toLowerCase().includes('тест')) { 
         // Імітація помилки для імені "Тест"
